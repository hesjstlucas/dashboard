...
export function canViewDepartmentDashboards(actorKey) {
  return getRank(actorKey).level >= 1;
}

export function canManageRecords(actorKey) {
  return getRank(actorKey).level >= 1;
}

export function canReviewApplications(actorKey) {
  return getRank(actorKey).level >= 3;
}

export function canManageDepartmentRanks(actorKey) {
  return getRank(actorKey).level >= 4;
}

export function canManageIntegrations(actorKey) {
  return getRank(actorKey).level >= 5;
}
...
export function abilitySummary(actorKey) {
  ...
  return {
    ...
    canViewDepartmentDashboards: canViewDepartmentDashboards(actorKey),
    canManageRecords: canManageRecords(actorKey),
    canReviewApplications: canReviewApplications(actorKey),
    canManageDepartmentRanks: canManageDepartmentRanks(actorKey),
    canManageIntegrations: canManageIntegrations(actorKey)
  };
}
